...
// 			
			// $(this).hover(function(){
				// $('#st-module-edit').attr({'href': url, 'target': 'blank'});
...
(function( $ ){
	...
	avatarMenu.responsive.toggleButton = function (){
		$('.avatar-nav-responsive .toggle').click(function()
		{
			...
			parent.find('ul.st-main-menu').slideToggle();			

			$(window).resize(function(){
				if($(window).width() > 1024) {
					parent.find('ul.st-main-menu').removeAttr('style');
				}
			});
		});
	}
...
(function( $ ){
	...
	$(document).ready(function(){
		...
		$('.st-learnmore').click(function(event){
			event.preventDefault();
			about = $('div.avatar-module.st-about-us');
			if (about) {
				$('body,html').animate({
		...
		$(window).scroll(function() {
			...
			if (viewTop > 50) {
				$('#avatar-header-inside-block').addClass('st-menu-scroll');
			} else {
				$('#avatar-header-inside-block').removeClass('st-menu-scroll');
			}
			$.each($elements, function(index, value){
